	...
	function sortableContainer(WrappedComponent) {
	  ...
	  return _temp = _class = function (_Component) {
	    ...
	    function _class(props) {
	      ...
	      _this._handleSortMove = function (e) {
	        _this.animateNodes();
	        _this.autoscroll();
	      ...
	      _this.handleSortEnd = function (e) {
	        ...
	        _this.updatePosition(e); // Make sure we have the right position for the helper

	        // For now, transition the helper to a position over the ghost.
	        if (transitionDuration) {
	          _this.helper.style[_utils.vendorPrefix + 'TransitionDuration'] = transitionDuration + 'ms';

	          var helperStart = _this.helper.getBoundingClientRect();
	          var helperDestination = _this.sortableGhost.getBoundingClientRect();
	          _this.helper.style[_utils.vendorPrefix + 'Transform'] = 'translate3d(' + (helperDestination.left - (helperStart.left - _this.translate.x)) + 'px,' + (helperDestination.top - (helperStart.top - _this.translate.y)) + 'px,0)';
	        }

	        // Remove helper after a transition back to place from the DOM
	        setTimeout(function (helper) {
	          if (_this.props.hideSortableGhost && _this.sortableGhost) {
	            _this.sortableGhost.style[_utils.vendorPrefix + 'Transform'] = '';
	            _this.sortableGhost.style[_utils.vendorPrefix + 'TransitionDuration'] = '';
	            _this.sortableGhost.style.visibility = '';
	            _this.sortableGhost.style.opacity = '';
	          }
	          helper.parentNode.removeChild(helper);
	        }.bind(_this, _this.helper), transitionDuration);

	        // This function might be pre-empted if the parent calls a re-render quickly.
	        _this.cleanupTimeout = setTimeout(_this.performCleanup, transitionDuration);
	      ...
	      _this.performCleanup = function () {
	        if (!_this.manager.active) return;
	        if (_this.cleanupTimeout) _this.cleanupTimeout = null;

	        // Remove styles as part of this cleanup (will be called before rerendering)
	        _this.manager.refs[_this.manager.active.collection].forEach(function (node) {
	          node.edgeOffset = null;

	          if (node.node === _this.sortableGhost) return; // For the ghost node, we'll do it when we remove the helper

	          node.node.style[_utils.vendorPrefix + 'Transform'] = '';
	          node.node.style[_utils.vendorPrefix + 'TransitionDuration'] = '';
	        });
	    ...
	    (0, _createClass3.default)(_class, [{
	      ...
	      value: function animateNodes() {
	        ...
	        var ghostOffset = nodes[this.index].edgeOffset || (nodes[this.index].edgeOffset = this.getEdgeOffset(nodes[this.index].node));
	        ...
	        if (this.sortableGhost) {
	          this.sortableGhost.style[_utils.vendorPrefix + 'Transform'] = 'translate3d(' + ghostTranslate.x + 'px,' + ghostTranslate.y + 'px,0)';
	        }
	      }
	    }, {
